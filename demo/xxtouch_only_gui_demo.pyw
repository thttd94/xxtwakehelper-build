...
def ensure_router_defaults(router):
    ...
    router.setdefault('random_start_minutes', '')
...
class XXTouchOnlyDemo(tk.Tk):
    ...
    def _controls(self, parent, router):
        ...
        saved_minutes = str(router.get('random_start_minutes') or '').strip()
        if not saved_minutes:
            try:
                saved_minutes = str(max(1, int(round(int(str(router.get('random_start_seconds', '800') or '800')) / 60))))
            except Exception:
                saved_minutes = '15'
        random_entry.insert(0, saved_minutes)
        ...
        ttk.Label(row1, text='phút', style='Sub.TLabel').pack(side='left', padx=(0, 8))
    ...
    def _save_random_start_settings(self, router):
        ...
        value = entry.get().strip() if entry else str(router.get('random_start_minutes', '') or '').strip()
        digits = ''.join(ch for ch in value if ch.isdigit())
        if digits:
            minutes = max(1, min(1440, int(digits)))
            router['random_start_minutes'] = str(minutes)
            router['random_start_seconds'] = str(minutes * 60)
        elif value == '':
            router['random_start_minutes'] = ''
    ...
    def _random_start_seconds(self, router):
        entry = router.get('_ui_random_start_entry')
        raw = entry.get().strip() if entry else str(router.get('random_start_minutes', '') or '').strip()
        try:
            minutes = max(1, int(raw))
            return minutes * 60
        except Exception:
            try:
                return max(1, int(router.get('random_start_seconds') or 900))
            except Exception:
                return 900
    ...
    def _open_ip_config_dialog(self):
        ...
        mapping_wrap = ttk.Frame(right, style='Card.TFrame')
        mapping_wrap.pack(fill='both', expand=True)
        mapping_wrap.rowconfigure(0, weight=1)
        mapping_wrap.columnconfigure(0, weight=1)
        mapping = tk.Text(mapping_wrap, height=16, relief='solid', borderwidth=1, font=('Consolas', 11), wrap='none')
        mapping_scroll_y = ttk.Scrollbar(mapping_wrap, orient='vertical', command=mapping.yview)
        mapping_scroll_x = ttk.Scrollbar(mapping_wrap, orient='horizontal', command=mapping.xview)
        mapping.configure(yscrollcommand=mapping_scroll_y.set, xscrollcommand=mapping_scroll_x.set)
        mapping.grid(row=0, column=0, sticky='nsew')
        mapping_scroll_y.grid(row=0, column=1, sticky='ns')
        mapping_scroll_x.grid(row=1, column=0, sticky='ew')
    ...
    def _run_parallel_rows(self, router, rows, task, action_name, per_success=None, ignore_random=False, record_success=True):
        ...
        def build_random_start_delays(total, max_delay):
            ...
            if total == 1 or span <= 0:
                return [0] * total
            step = span / float(total - 1)
            jitter = min(step * 0.35, step / 2.0)
            delays = []
            for idx in range(total):
                base = idx * step
                offset = random.uniform(-jitter, jitter) if 0 < idx < total - 1 else 0
                delays.append(max(0, min(span, base + offset)))
