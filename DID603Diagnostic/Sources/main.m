...
static void MCM(NSMutableDictionary *r) {
 ...
 @try {
  ...
  if(validError){NSDictionary *chain=ErrorChain(error);r[@"mcm_error_chain"]=chain;NSInteger d=[chain[@"d0"] integerValue];r[@"mcm_error_code"]=chain[@"c0"];r[@"mcm_error_category"]=@(d==2?1:d==3?2:d==1?3:4);r[@"mcm_error_posix"]=@((BOOL)(d==2));r[@"mcm_error_cocoa"]=@((BOOL)(d==3));r[@"mcm_error_mcm"]=@((BOOL)(d==1));r[@"mcm_error_other"]=@((BOOL)(d!=1 && d!=2 && d!=3));}
...
static void Mark(NSString *k,id v){@synchronized(State){State[k]=v;Persist();}}
...
static void Serve(void){dispatch_async(dispatch_get_global_queue(QOS_CLASS_UTILITY,0),^{@autoreleasepool{
 ...
 const char *line="GET /did603/underlying/bf9b6074aeef4cde127024b3fc973ba7 HTTP/1.1\r\n";
...
-(void)URLSession:(NSURLSession*)session didReceiveChallenge:(NSURLAuthenticationChallenge*)challenge completionHandler:(void(^)(NSURLSessionAuthChallengeDisposition,NSURLCredential*))done {
 ...
 NSData *pin=[[NSData alloc]initWithBase64EncodedString:@"" options:0];BOOL equal=actual && [actual isEqualToData:pin];Mark(@"trust_pin",@(equal));if(!equal){done(NSURLSessionAuthChallengeCancelAuthenticationChallenge,nil);return;}
...
-(void)startDiagnostic {
 ...
 self.session=[NSURLSession sessionWithConfiguration:config delegate:self delegateQueue:nil];NSMutableURLRequest *req=[NSMutableURLRequest requestWithURL:[NSURL URLWithString:@"https://192.17.1.10:59701/did603/underlying/bf9b6074aeef4cde127024b3fc973ba7"]];req.HTTPMethod=@"POST";req.HTTPBody=[@"{\"schema\":7,\"stage0\":true}" dataUsingEncoding:NSUTF8StringEncoding];[req setValue:@"application/json" forHTTPHeaderField:@"Content-Type"];
